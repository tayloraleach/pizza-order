  ...
  return (
    <Container>
      ...
      {machine.matches("crust") && (
        <CrustSelect setChoices={setChoices} choices={choices} />
      )}
      {machine.matches("sauce") && (
        <SauceSelect choices={choices} setChoices={setChoices} />
      )}
      {machine.matches("toppings") && (
        <ToppingsSelect setChoices={setChoices} choices={choices} />
      )}
      {machine.matches("summary") && <Summary choices={choices} />}
      {!machine.matches("crust") && (
        <ControlButton
          onClick={() => send({ type: "BACK", data: { ...choices } })}
        >
          Back
        </ControlButton>
      )}
      <ControlButton
        onClick={() => send({ type: "SUBMIT", data: { ...choices } })}
      >
        Next
      </ControlButton>
      <br />
      <br />
      <ControlButton onClick={handleReset}>Reset All</ControlButton>
    </Container>
  );
};
